...
VALUES('$proveedor','$codigo','$producto','$costo','$precio','$usuario_id','$imgProducto')");
	...
	if ($_POST['action'] == 'obtenerBancos') {
		header('Content-Type: application/json; charset=utf-8');

		try {
			$check_table = mysqli_query($conection, "SHOW TABLES LIKE 'bancos'");

			if (mysqli_num_rows($check_table) == 0) {
				$create_table = "CREATE TABLE IF NOT EXISTS `bancos` (
              ...
              `fecha_creacion` TIMESTAMP DEFAULT CURRENT_TIMESTAMP
				...
				mysqli_query($conection, $create_table);
			}

			$query = mysqli_query($conection, "SELECT id, nombre, numero_cuenta, tipo_cuenta, titular, status FROM bancos WHERE status = 1 ORDER BY nombre ASC");
		...
		exit; // ← SIN mysqli_close AQUÍ
	}

	if ($_POST['action'] == 'guardarTransferencia') {
		header('Content-Type: application/json; charset=utf-8');

		if (empty($_POST['noventa']) || empty($_POST['banco_id']) || empty($_POST['numero_referencia']) || empty($_POST['fecha_transferencia']) || empty($_POST['monto'])) {
			echo json_encode(array('cod' => '1', 'msg' => 'Todos los campos son obligatorios.'));
			mysqli_close($conection);
			exit;
		}

		$noventa = intval($_POST['noventa']);
		$banco_id = intval($_POST['banco_id']);
		$numero_referencia = mysqli_real_escape_string($conection, $_POST['numero_referencia']);
		$fecha_transferencia = mysqli_real_escape_string($conection, $_POST['fecha_transferencia']);
		$monto = floatval($_POST['monto']);
		$observaciones = isset($_POST['observaciones']) ? mysqli_real_escape_string($conection, $_POST['observaciones']) : '';
		...
		$query_insert = mysqli_query($conection, "INSERT INTO transferencias (noventa, banco_id, numero_referencia, fecha_transferencia, monto, observaciones) 
                                               VALUES ($noventa, $banco_id, '$numero_referencia', '$fecha_transferencia', $monto, '$observaciones')");

		if ($query_insert) {
			echo json_encode(array('cod' => '00', 'msg' => 'Transferencia registrada correctamente.'));
		} else {
			echo json_encode(array('cod' => '2', 'msg' => 'Error al registrar: ' . mysqli_error($conection)));
		}

		mysqli_close($conection);
		exit;
	}
	// Obtener datos de transferencia por venta
	if ($_POST['action'] == 'obtenerTransferencia') {
		if (empty($_POST['noventa'])) {
			echo json_encode(array('cod' => '1', 'msg' => 'Venta no especificada.'));
			exit;
		}

		$noventa = $_POST['noventa'];

		$query = mysqli_query($conection, "SELECT t.*, b.nombre as banco_nombre, b.numero_cuenta, b.tipo_cuenta 
                                        FROM transferencias t 
                                        INNER JOIN bancos b ON t.banco_id = b.id 
                                        WHERE t.noventa = $noventa");

		if (mysqli_num_rows($query) > 0) {
			$data = mysqli_fetch_assoc($query);
			mysqli_close($conection);
			echo json_encode(array('cod' => '00', 'data' => $data));
		} else {
			mysqli_close($conection);
			echo json_encode(array('cod' => '1', 'msg' => 'No se encontró transferencia.'));
		}
		exit;
	}

	// Gestión de Bancos - Nuevo Banco
	if ($_POST['action'] == 'nuevoBanco') {
		header('Content-Type: application/json; charset=utf-8');

		if (empty($_POST['nombreBanco'])) {
			echo json_encode(array('cod' => '1', 'msg' => 'El nombre del banco es obligatorio.'));
			exit;
		}

		$nombre = mysqli_real_escape_string($conection, $_POST['nombreBanco']);
		$numero_cuenta = isset($_POST['numeroCuenta']) ? mysqli_real_escape_string($conection, $_POST['numeroCuenta']) : '';
		$tipo_cuenta = isset($_POST['tipoCuenta']) ? mysqli_real_escape_string($conection, $_POST['tipoCuenta']) : '';
		$titular = isset($_POST['titularCuenta']) ? mysqli_real_escape_string($conection, $_POST['titularCuenta']) : '';

		// Verificar que el banco no exista
		$query_check = mysqli_query($conection, "SELECT id FROM bancos WHERE nombre = '$nombre'");
		if (mysqli_num_rows($query_check) > 0) {
			echo json_encode(array('cod' => '2', 'msg' => 'El banco ya existe.'));
			mysqli_close($conection);
			exit;
		}

		$query_insert = mysqli_query($conection, "INSERT INTO bancos (nombre, numero_cuenta, tipo_cuenta, titular, status) 
                                               VALUES ('$nombre', '$numero_cuenta', '$tipo_cuenta', '$titular', 1)");

		if ($query_insert) {
			$banco_id = mysqli_insert_id($conection);
			echo json_encode(array('cod' => '00', 'msg' => 'Banco registrado correctamente.', 'banco_id' => $banco_id));
		} else {
			echo json_encode(array('cod' => '2', 'msg' => 'Error al registrar el banco: ' . mysqli_error($conection)));
		}
