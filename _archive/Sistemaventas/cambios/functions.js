$(document).ready(function () {
    ...
    $('#nuevoCliente').click(function (e) {
        ...
        $('.bodyModal').html('<form action="" method="post" name="form_add_product" id="form_add_product" onsubmit="event.preventDefault(); nuevoCliente();">' +
            ...
            '<input type="text" name="nitCliente" id="nitCliente" value="" placeholder="Ruc" required><br>' +
...
$(document).on('change', '#tipo_pago', function () {
    ...
    if (tipoPago == '2') { // 2 = Transferencia
        mostrarModalTransferencia();
    }
});

// Función para mostrar modal de transferencia
function mostrarModalTransferencia() {
    // Cargar bancos si no están cargados
    if (bancosList.length == 0) {
        cargarBancos();
        setTimeout(mostrarModalTransferencia, 500);
        return;
    }

    // Crear opciones de bancos
    var optionsBancos = '<option value="">Seleccione un banco</option>';
    bancosList.forEach(function (banco) {
        optionsBancos += '<option value="' + banco.id + '">' + banco.nombre + '</option>';
    });
    ...
    $('.bodyModal').html(
        ...
        '<label style="text-align:left; display:block; margin-top:15px;">Banco:</label>' +
        '<select name="banco_id" id="banco_id" required style="width:100%; padding:10px;">' +
        optionsBancos +
        '</select>' +

        '<div id="info_banco" style="background:#f0f0f0; padding:10px; margin:10px 0; border-radius:5px; display:none;"></div>' +

        '<label style="text-align:left; display:block; margin-top:15px;">Número de Referencia:</label>' +
        '<input type="text" name="numero_referencia" id="numero_referencia" placeholder="Ej: TRF-123456" required>' +
        ...
        '<label style="text-align:left; display:block; margin-top:15px;">Monto:</label>' +
        '<input type="number" step="0.01" name="monto" id="monto" placeholder="0.00" required>' +

        '<label style="text-align:left; display:block; margin-top:15px;">Observaciones (Opcional):</label>' +
        '<textarea name="observaciones" id="observaciones" rows="3" placeholder="Notas adicionales..." style="width:100%; padding:10px;"></textarea>' +
        ...
        '<a href="#" onclick="event.preventDefault(); mostrarFormNuevoBanco();" style="color:#007bff; text-decoration:none;">' +
        '<i class="fas fa-plus-circle"></i> Agregar nuevo banco' +
        '</a>' +
        '</div>'
    );
    ...
    // Event listener para mostrar info del banco
    $('#banco_id').on('change', function () {
        var bancoId = $(this).val();
        if (bancoId) {
            var banco = bancosList.find(b => b.id == bancoId);
            if (banco) {
                $('#info_banco').html(
                    '<strong>Información del Banco:</strong><br>' +
                    'Cuenta: ' + (banco.numero_cuenta || 'N/A') + '<br>' +
                    'Tipo: ' + (banco.tipo_cuenta || 'N/A') + '<br>' +
                    'Titular: ' + (banco.titular || 'N/A')
                ).show();
            }
        } else {
            $('#info_banco').hide();
...
function guardarTransferencia() {
    ...
    if (!banco_id || !numero_referencia || !fecha_transferencia || !monto) {
        $('.alertAddProduct').html('<p style="color:red;">Todos los campos son obligatorios.</p>');
        return false;
    }
...
// Modificar función facturar() existente para incluir transferencia
var facturarOriginal = window.facturar;
window.facturar = function () {
    var rows = $('#detalle_venta tr').length;
    if (rows > 0) {
        var tipoPago = $('#tipo_pago').val();

        // Si es transferencia, procesar primero la venta y luego guardar transferencia
        if (tipoPago == '2') { // 2 = Transferencia
            var transferenciaDatos = sessionStorage.getItem('transferencia_pendiente');

            if (!transferenciaDatos) {
                alert('Por favor complete los datos de transferencia primero.');
                mostrarModalTransferencia();
                return;
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    console.log('Respuesta procesar venta:', response);

                    if (response != 'error') {
                        try {
                            var info = typeof response === 'string' ? JSON.parse(response) : response;
                            var noventa = info.noventa;

                            // Guardar transferencia
                            var transfData = JSON.parse(transferenciaDatos);
                            transfData.noventa = noventa;
                            transfData.action = 'guardarTransferencia';

                            console.log('Datos transferencia a guardar:', transfData);
                            ...
                            $.ajax({
                                ...
                                dataType: 'json',
                                success: function (response) {
                                    console.log('Respuesta guardar transferencia:', response);

                                    // ✅ CORRECCIÓN: Cambiar 'result' por 'response'
                                    if (response.cod == '00') {
                                        ...
                                        alert('Error al guardar transferencia: ' + response.msg);
                                    }
                                },
                                error: function (xhr, status, error) {
                                    console.error('Error AJAX guardar transferencia:', {
                                        status: status,
                                        error: error,
                                        response: xhr.responseText
                                    });
                                    alert('Error al guardar transferencia: ' + error);
                                }
                            });
                        } catch (e) {
                ...
                error: function (xhr, status, error) {
                    console.error('Error AJAX procesar venta:', {
                        status: status,
                        error: error,
                        response: xhr.responseText
                    });
            ...
            // Llamar función original para otros tipos de pago
            if (typeof facturarOriginal === 'function') {
                facturarOriginal();
            } else {
                // Si no existe función original, implementar lógica básica
                procesarVentaNormal();
            }
        }
    }
};
function procesarVentaNormal() {
    var action = 'procesarVenta';
    var codcliente = $('#idcliente').val();
    var tipoPago = $('#tipo_pago').val();
    var descuento = $('#descuneto_venta').val();
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response != 'error') {
                ...
                console.log('Error al procesar venta');
            }
        },
        error: function (error) {
...
// ============================================
$(document).ready(function () {
    // Limpiar datos pendientes de transferencia al cargar
    if (sessionStorage.getItem('transferencia_pendiente')) {
        console.log('Datos de transferencia pendientes encontrados');
    }
...
// Función para mostrar formulario de nuevo banco
function mostrarFormNuevoBanco() {
    $('.bodyModal').html(
        '<form action="" method="post" id="form_nuevo_banco" onsubmit="event.preventDefault(); guardarNuevoBanco();">' +
        '<h1><i class="fas fa-university" style="font-size: 45pt;"></i><br>Nuevo Banco</h1>' +
        '<input type="hidden" name="action" value="nuevoBanco">' +

        '<label style="text-align:left; display:block; margin-top:15px;">Nombre del Banco:</label>' +
        '<input type="text" name="nombreBanco" id="nombreBanco" placeholder="Ej: Banco Continental" required>' +

        '<label style="text-align:left; display:block; margin-top:15px;">Número de Cuenta:</label>' +
        '<input type="text" name="numeroCuenta" id="numeroCuenta" placeholder="1234567890">' +

        '<label style="text-align:left; display:block; margin-top:15px;">Tipo de Cuenta:</label>' +
        '<select name="tipoCuenta" id="tipoCuenta" style="width:100%; padding:10px;">' +
        '<option value="">Seleccione</option>' +
        '<option value="Cuenta Corriente">Cuenta Corriente</option>' +
        '<option value="Caja de Ahorro">Caja de Ahorro</option>' +
        '</select>' +

        '<label style="text-align:left; display:block; margin-top:15px;">Titular:</label>' +
        '<input type="text" name="titularCuenta" id="titularCuenta" placeholder="Nombre del titular">' +

        '<div class="alert alertAddProduct"></div>' +

        '<div style="margin-top:20px; text-align:center;">' +
        '<a href="#" class="btn_cancel" onclick="mostrarModalTransferencia();"><i class="fas fa-arrow-left"></i> Volver</a> ' +
        '<button type="submit" class="btn_new"><i class="fas fa-save"></i> Guardar Banco</button>' +
        '</div>' +
...
// Función para guardar nuevo banco
function guardarNuevoBanco() {
    ...
    if (!nombreBanco) {
        $('.alertAddProduct').html('<p style="color:red;">El nombre del banco es obligatorio.</p>');
        return false;
    }
    ...
    $.ajax({
        url: 'ajax.php',
        type: 'POST',
        data: $('#form_nuevo_banco').serialize(),
        dataType: 'json',
        success: function (response) {
            if (response.cod == '00') {
                $('.alertAddProduct').html('<p style="color:green;">' + response.msg + '</p>');
                ...
                $('.alertAddProduct').html('<p style="color:red;">' + response.msg + '</p>');
            }
        },
        error: function (error) {
            console.log('Error:', error);
            $('.alertAddProduct').html('<p style="color:red;">Error al guardar el banco.</p>');
        }
    });
}
