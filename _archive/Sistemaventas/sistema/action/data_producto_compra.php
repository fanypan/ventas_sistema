		...
		$result_conf = mysqli_num_rows($query_conf);
			...
			$total_pagina = ceil($total_registro / $por_pagina);
				...
				while ($data = mysqli_fetch_assoc($query)){
					...
					$tabla .= '<a href="#" onclick="event.preventDefault(); infoProductAgregarCompra('.$data['codproducto'].');" ><table>
												...
												<tr style="background:none; color: black;">
													<td class="textright" id="txt_precio_2'.$data['codproducto'].'">'.formatCant($data['costo']).'</td>
												</tr>
											</table>										
								</a>';
				}
