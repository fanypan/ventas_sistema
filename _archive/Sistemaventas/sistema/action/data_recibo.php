<?php 
	...
	//print_r($_POST);
		...
		$usuario = $_SESSION['idUser'];
			...
			//print_r($_POST);
			$sql_registe = mysqli_query($conection,"SELECT COUNT(*) as total_registro FROM 																detalle_recibo WHERE 															noventa = $busqueda");
			...
			$query_venta = mysqli_query($conection,"SELECT codcliente FROM venta WHERE noventa = $busqueda");
			...
			if ($result_venta > 0) {
				...
				$query = mysqli_query($conection,"SELECT det.id,det.noventa,det.fecha,det.saldo_anterior,det.cantidad,det.saldo_actual,u.nombre 
															FROM detalle_recibo det
															INNER JOIN usuario u
															ON det.usuario = u.idusuario
															WHERE noventa = $busqueda");
			...
			$total_pagina = ceil($total_registro / $por_pagina);
				...
				if ($result > 0) {
					...
					while ($data = mysqli_fetch_array($query)) {
						...
						$detalleTabla.='<tr id="row_'.$data['id'].'">
							...
							<td>'.$fecha.'</td>
							<td>'.$data['nombre'].'</td>
							<td>'.$moned.' '.formatCant($data['saldo_anterior']).'</td>
							<td>'.$moned.' '.formatCant($data['cantidad']).'</td>
							<td>'.$moned.' '.formatCant($data['saldo_actual']).'</td>
							<td class="textcenter">
				...
				if ($pagina > 1) {
					$lista.= '<li><a href="javascript:listaRecibos();"><i class="fas fa-step-backward"></i></a></li>
				<li><a href="javascript:listaRecibos('.($pagina-1).');"><i class="fas fa-caret-left"></i></a></li>';
				}
				...
				{ 
						...
						if ($i == $pagina) 
						{
							...
							$lista.= '<li><a href="javascript:listaRecibos('.$i.');">'.$i.'</a></li>';
						}
					}

				if ($pagina < $total_pagina) {
					$lista.= '<li><a href="javascript:listaRecibos('.($pagina+1).');"><i class="fas fa-caret-right"></i></a></li>
				<li><a href="javascript:listaRecibos('.($pagina=$total_pagina).');"><i class="fas fa-step-forward"></i></a></li>';
				}
				$lista.='</ul>';
