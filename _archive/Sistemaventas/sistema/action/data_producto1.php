			...
			$total_pagina = ceil($total_registro / $por_pagina);
				...
				while ($data = mysqli_fetch_assoc($query)){
					if ($data['foto'] != 'img_producto.png') {
						...
						$costo = formatCant($data['costo']);
						$precio = formatCant($data['precio']);
					...
					$tabla.='<td class=""> <!--<a class="link_add add_product" href="#" onclick="event.preventDefault();
										...
										|
										<a class="link_edit" style="background:#5c6bc0;" href="#" onclick="event.preventDefault(); abrirImpresionBarras(\''.$data['codigo'].'\', \''.addslashes($data['descripcion']).'\', \''.$moned.' '.$precio.'\');"><i class="fas fa-barcode"></i> Barras</a>';	                
					}
