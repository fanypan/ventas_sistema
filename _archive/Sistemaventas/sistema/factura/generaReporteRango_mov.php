<?php 
	...
	//print_r($_POST);
		...
		if ($_SESSION['rol'] == 1 || $_SESSION['rol'] == 2 ){
			...
			$query_cliente_info = mysqli_query($conection, "SELECT nombre, telefono FROM cliente WHERE idcliente = $busqueda");
			$query_cliente = mysqli_fetch_assoc($query_cliente_info);

			// Calcular Saldo Anterior (antes de la fecha inicial)
			$query_saldo_anterior = mysqli_query($conection, "SELECT SUM(totalventa) as total_ventas, SUM(abono) as total_abonos 
																FROM venta 
																WHERE codcliente = $busqueda 
																AND fecha < '{$f_de}' 
																AND (status = 3 OR status = 4)");
			$data_saldo_anterior = mysqli_fetch_assoc($query_saldo_anterior);
			$saldo_anterior = $data_saldo_anterior['total_ventas'] - $data_saldo_anterior['total_abonos'];
			...
			ob_start();
		    include(__DIR__.'/reportePdfRango_mov.php');
		    $html = ob_get_clean();
