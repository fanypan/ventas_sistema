			...
			<tbody id="detalle_productos">
				...
				if ($result > 0) {
					...
					while ($data = mysqli_fetch_array($query)) {
						...
						$totalfactura= formatCant($data["totalcompra"]);
						...
						if ($totalfact == 0 ) {
							...
							$factura = $moned.' '.formatCant($totalfact);
						}

						if ($abono == 0) {
							$info_abono = '';
						}else{
							$info_abono = $moned.' '.formatCant($abono);
						}
						...
						?>
							...
							<td class="totalfactura"><?php echo $moned.' '.formatCant($ventas_totales); ?></td>
							</tr>;
