	...
	<?php 
		...
		if ($result_caja > 0) {
				...
				if ($result_das > 0) {
					...
					$inicio = $data_dash['inicios'] ?? 0;
					$ventas = $data_dash['ventas'] ?? 0;
					$abonos = $data_dash['abonos'] ?? 0;
					$creditos = $data_dash['credito'] ?? 0;
					$egreso = $data_dash['egreso'] ?? 0;
					$total = $inicio + $ventas + $abonos - $egreso;
					mysqli_close($conection);
				}
				
				$inicio   = formatCant($inicio);
				$ventas   = formatCant($ventas);
				$abonos   = formatCant($abonos);
				$creditos = formatCant($creditos);
				$egreso   = formatCant($egreso);
				$total    = formatCant($total);
			 		}
