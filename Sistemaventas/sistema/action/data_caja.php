		...
		$usuario = $_SESSION['idUser'];
			...
			$total_pagina = ceil($total_registro / $por_pagina);
				...
				while ($data = mysqli_fetch_assoc($query)){
					...
					$detalleTabla .= '<tr>
						                <td>'.$data['fecha'].'</td>
						                <td class="">'.$moned.' '.formatCant($data['inicio']).'</td>
						                <td class="">'.$moned.' '.formatCant($data['ventas']).'</td>
						                <td class="">'.$moned.' '.formatCant($data['abonos']).'</td>
						                <td class="">'.$moned.' '.formatCant($data['creditos']).'</td>
						                <td class="">'.$moned.' '.formatCant($data['egresos']).'</td>
						                <td class="">'.$moned.' '.formatCant($data['total_efectivo']).'</td>
